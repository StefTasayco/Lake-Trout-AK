...
-   ***Scenario** 𝛂* : Fit to Pmax. Using @fig-predeq2 from @stewart_energetics_1983 .
...
-   ***Scenario** 𝛃* : Fit to final weight. Final weight = the weight at the next older age from NPS lake trout data.
...
This is consistent with @pazzia_influence_2002, which states that “during the summer, lake trout were assumed to forage in deeper water (closer to 10 C). In winter, they were assumed to forage closer to the surface.
...
So far, I don't have any tables to share but if I do, they will be here.
...
**Year-long Lake Trout Growth by Age**: The following plots show the output of the design profile created using Scenario 1, scenario a, and scenario beta. The fish were fit to a final weight calculated using the weight at the next older age. Finally, the mean of each age of fish was taken, as well as 3 SDs above the mean and 3 SDs below the mean.

```{r}
#| label: fig-temp-weight
#| fig-cap: "The average yearly growth of the entire cohort plotted with the temperature inputs for 365 days in 2020."
#| fig-align: "center"
#| echo: false

knitr::include_graphics("pics/temp_weight.png")
```

```{r}
# excel output data
scen1_output <- read_excel("data/LAC_Scen1a_beta_Daily_Output.xlsx")

run_names <- readxl::read_excel("data/metadata.xlsx")
run_names <- run_names |>
  mutate(
    Run_name = Run_name |>
      as_factor() |>
...
scen1_output_clean <- scen1_output |>
  left_join(
    run_names,
    by = "Run"
  )
#prefixes to include in function for making daily growth plots
age_prefixes <- c("Age 12", "Age 13", "Age 14", "Age 18", "Age 23")

library(stringr)
library(purrr)

age_growth_plots <- age_prefixes |>
  set_names() |>
  purrr::map(function(prefix) {
    df <- scen1_output_clean |>
      filter(stringr::str_starts(as.character(Run_name), prefix))

    if (nrow(df) == 0) {
      return(
        ggplot() +
          labs(title = paste(prefix, "- no matching runs found")) +
          theme_minimal()
      )
    }

    p <- df |>
      ggplot(aes(x = Day, y = Weight.g, color = Run_name, group = Run_name)) +
      geom_line(linewidth = 0.8) +
      scale_color_manual(values = my_paired) +
      labs(
        title = paste(prefix, "— daily growth"),
        x = "Day",
        y = "Weight (g)",
        color = "Run name"
      ) +
...
```{r}
avg_growth_rate_per_day <- scen1_output_clean |>
  mutate(
    Run_name = as.character(Run_name),
    age_prefix = stringr::str_extract(Run_name, "^Age\\s*\\d+")
  ) |>
  filter(age_prefix %in% age_prefixes) |>
  group_by(age_prefix, Day) |>
  summarize(
    mean_sgr = mean(`Specific.Growth.Rate.J.g.d`, na.rm = TRUE),
    sd_sgr = sd(`Specific.Growth.Rate.J.g.d`, na.rm = TRUE),
    n_obs = sum(!is.na(`Specific.Growth.Rate.J.g.d`)),
    se_sgr = ifelse(n_obs > 1, sd_sgr / sqrt(n_obs), NA_real_),
    .groups = "drop"
  )

#doing the same thing as above but in g/g/d instead of J/g/d
avg_growth_rate_ggd <- scen1_output_clean |>
  mutate(
    Run_name = as.character(Run_name),
    age_prefix = stringr::str_extract(Run_name, "^Age\\s*\\d+")
  ) |>
  ...
  group_by(age_prefix, Day) |>
  summarize(
    mean_sgr = mean(`Specific.Growth.Rate.g.g.d`, na.rm = TRUE),
    sd_sgr = sd(`Specific.Growth.Rate.g.g.d`, na.rm = TRUE),
    n_obs = sum(!is.na(`Specific.Growth.Rate.g.g.d`)),
    se_sgr = ifelse(n_obs > 1, sd_sgr / sqrt(n_obs), NA_real_),
    .groups = "drop"
  )
```
...
#| fig-cap: This plot shows the mean growth rate per age for one year in J/g/d. All standard deviations were averaged per day per age.
#| fig-cap-location: top

growth_rate <- avg_growth_rate_per_day |>
  ggplot() +
  ...
  labs(
    ...
    color = "Age of fish",
    title = "Lake Trout Daily Growth Rate (energy) in 2020"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(face = "bold", hjust = 0.5)
...
```{r}
#| label: fig-growth-ggd
#| fig-cap: This plot shows the mean growth rate per age for one year in g/g/d. All standard deviations were averaged per day per age.
#| fig-cap-location: top
...
growth_rate_ggd <- avg_growth_rate_ggd |>
  ...
  labs(
    ...
    color = "Age of fish",
    title = "Lake Trout Daily Growth Rate (mass) in 2020"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(face = "bold", hjust = 0.5)
...
## FB4 Output Conclusions

1.  Some lower 3 SDs were excluded because they went negative.

2.  I believe the model could not calculate the growth of some runs because their final weight (the weight at the next older age) was a lot lower than their starting weight. Model might not understand why the fish lost so much weight in one year.

    1.  Limitations of using observational data!

3.  The average p-value for the entire cohort is 0.6888.
